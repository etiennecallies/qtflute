...
void MasterThread::run()
{
    ...
    while (!quit) {
        ...
        if(currentRequest == new QString("start")){
            voltage(ARDUINO_OFF);
            engineOn();
        }
        else if(currentRequest == new QString("stop")){
            voltage(ARDUINO_OFF);
            engineOff();
        }
        else if(currentRequest == new QString("home")){
            homing();
        }
        ...
        else if(currentRequest == new QString("volt")){
            voltage(ARDUINO_ON);
        }
        else if(currentRequest == new QString("volt0")){
            voltage(ARDUINO_OFF);
        }
/*        else {
...
void MasterThread::gotoPosition(int pos, int maxvel, int maxacc, int maxdec){
    //emit this->disable();
    send(MOTOR, MasterThread::gotoSequence(
...
void MasterThread::voltage(char volt){
...
QByteArray MasterThread::voltSequence(char volt){
...
QByteArray MasterThread::gotoSequence(int position, int maxVelocity, int maxAcc, int maxDec){
